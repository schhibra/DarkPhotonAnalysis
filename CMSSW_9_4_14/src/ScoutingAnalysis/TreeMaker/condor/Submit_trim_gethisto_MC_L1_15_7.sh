...
for datasetpath in `less datasetpaths_${workdir}.txt`;do
    ...
    OutDataset=`echo $datasetpath | awk -F "tree_" '{print "trim_L1_15_7_"$2}'`
    #echo "OutDataset" $OutDataset
    
    cat trim_gethisto_MC_L1_15_7.C | sed "s?DATASET?${datasetpath}?g" | sed "s?OUTPUT?$jobDir/histos/${OutDataset}?g" | sed "s?trim_gethisto_MC_L1_15_7(?trim_gethisto_MC_L1_15_7_tmp(?g" > dir_$workdir/trim_gethisto_MC_L1_15_7_tmp.C 

    cd dir_$workdir
    root -l -b -q trim_gethisto_MC_L1_15_7_tmp.C 
    cd -
